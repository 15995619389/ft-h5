...
export default {
    ...
    methods: {
        Config: function () {
            this.$data.nativeShare = new NativeShare()

            var shareDesc = ''
            if ($('#description').length > 0) {
                shareDesc = $('#description')[0].content
            }
            var shareData = {
                //图标无效存在
                //icon:'',
            ...
            this.$data.nativeShare.setShareData(shareData)
        },
        Show: function () {
            var self = this
            self.Config()
...
.wx-tip-ms {
  background: url(https://gss0.bdstatic.com/7051cy7z2RZ3otebn9fN2DJv/static/m-common/widget/js/logic/share/img/b_8489c9b.png)
    no-repeat;
  width: 165px;
  height: 70px;
