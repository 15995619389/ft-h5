...
var vm = new Vue({
    ...
    methods: {
            ...
            client.Request({
                ...
                success: function (result) {
                    if (result.StausCode != "203") {
                        ...
                        $(".applySupplier").css({ "display": "block" });
                        var resultObj = result.DataSource;
                        self.$data.PerType = resultObj.PerOrDeptType;
                        if (self.$data.PerType == 1) {
                            self.$data.Personal = resultObj;
                            self.ConvertMultiOptionsToArray(self.$data.Personal);
                        } else {
                            self.$data.Enterprise = resultObj;
                            self.ConvertMultiOptionsToArray(self.$data.Enterprise);
                        }
                    }
                },
                complete: function () {
        ...
        ConvertMultiOptionsToArray: function (model) {
            if (model.ExpertsServiceIds && model.ExpertsServiceIds.length > 0) {
                model.ExpertsServiceIds = model.ExpertsServiceIds.split(",");
            } else {
                model.ExpertsServiceIds = [];
            }
            model.ScientificContent=client.Filters.html(model.ScientificContent);
            this.$data.DetyName = model.ExpertDutyName;
            this.$data.Address = model.ExpertProvinceName + " " + model.ExpertCityName;
        },
